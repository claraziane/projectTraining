...
for iMovement = 1:2
    ...
    if iMovement == 1
        Conditions   = {'preTapSP'; 'postTapSP'; 'preTapSP'; 'postTapSP';...
                        'preTapST'; 'postTapST'; 'preTapST'; 'postTapST';...
                        'preTapDT'; 'postTapDT'; 'preTapDT'; 'postTapDT'};

    else
        Conditions   = {'preWalkSP'; 'postWalkSP'; 'preWalkSP'; 'postWalkSP';...
                        'preWalkST'; 'postWalkST'; 'preWalkST'; 'postWalkST';...
                        'preWalkDT'; 'postWalkDT'; 'preWalkDT'; 'postWalkDT'};

    end
    CV = nan(length(Participants), length(Conditions), length(Game));
    ...
    for iGame = 1:length(Game)
        ...
        for iParticipant = 1:length(Participants)
            ...
            load([pathImport  '/Behavioural/resultsBehav.mat']);
            ...
            if strcmpi(Group, Game{iGame})
                ...
                for iCondition = [1 2 5 6 9 10]
                    CV(iParticipant, iCondition, iGame) = resultsBehav.(Conditions{iCondition}).imiCV;
                end
                ...
                for iCondition = [3 4 7 8 11 12]
                    CV(iParticipant, iCondition, gameIndex) = resultsBehav.(Conditions{iCondition}).imiCV;
                end
    ...
    for iGame = 1:length(Game)
        CV(:,:,iGame) = removeOutliers(CV(:,:,iGame));

        plotScatter(CV(:,:,iGame),[], [xLabels(iGame,:)]', 'Coefficient of Variation', 4);
