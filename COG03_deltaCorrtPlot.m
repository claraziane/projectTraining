...
figTitles   = {'TapST__RW';  'TapDT__RW';  'WalkST__RW';  'WalkDT__RW';...
              'TapST__FB';  'TapDT__FB';  'WalkST__FB';  'WalkDT__FB';};


varX = {'Flexibility'; 'Inhibition'; 'workingMemory'}; 
varY = {'power'; 'phaseR'; 'stabilityIndex'; 'imiMean'; 'imiCV'; 'phaseDegMean'; 'resultantLength'};

xLabels = {'Flexibility'; 'Inhibition'; 'Working Memory'}';
yLabels = {'Power (SNR)'; 'Phase Coupling'; 'Stability Index (Hz)'; 'Inter-Movement Interval (ms)'; 'Coefficient of Variation_{Inter-Movement Interval}'; 'Synchronization Accuracy (°)'; 'Synchronization Consistency (logit)'};
...
for iX = 1:length(varX)
    ...
    for iY = 1:length(varY)
        ...
        for iSession = 1:length(Sessions)
            ...
            for iCondition = 1:length(Conditions)
                ...
                for iParticipant = 1:length(Participants)
                    ...
                    dataX(iParticipant,iCondition, iSession) = resultsCog.(varX{iX});
        ...
        deltaY = dataY(:,2:2:end,:) - dataY(:,1:2:end-1,:);
        dataX(:,1:2:end,:) = [];
