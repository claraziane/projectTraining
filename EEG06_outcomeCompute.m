...
for iParticipant = 1:length(Participants)
    ...
    for iSession = 1%length(Sessions)
        ...
        pathParticipant = [pathResults Participants{iParticipant} '/' Sessions{iSession} '/EEG/'];

        % Load stimuli info
        load([pathData Participants{iParticipant} '/'  Sessions{iSession} '/Behavioural/dataRAC.mat']);

        if exist([pathResults Participants{iParticipant} '/'  Sessions{iSession} '/EEG/resultsEEG.mat'], 'file')
