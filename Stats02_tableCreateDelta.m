...
factorGame     = {'RW'; 'FB'};
factorTime     = {'pre'; 'post'};
factorMovement = {'Tap';  'Walk'}; % 'Walk'
factorLoad     = {'ST'; 'DT'}; %'SP'; 'ST'; 
                    ...
                    if iTime == 1
                        ...
                        if ~strcmpi(factorLoad(iLoad), 'SP')
                            ...
                            if strcmpi(resultsEEG.(condition{1,1}).compKeep, 'N')
                                ...
                                prePower  = resultsEEG.(condition{1,1}).power;
                                preSI  = resultsEEG.(condition{1,1}).stabilityIndex;

                                % Logit transformation
                                phase = [];
                                phase = resultsEEG.(condition{1,1}).phaseR;
                                preCoupling = log(phase ./ (1-phase));
                            end
                        ...
                        if ~strcmpi(factorLoad(iLoad), 'SP')
                            ...
                            if strcmpi(resultsEEG.(condition{1,1}).compKeep, 'N')
                                ...
                                power  = [power; resultsEEG.(condition{1,1}).power - prePower];
                                stabilityIndex  = [stabilityIndex; resultsEEG.(condition{1,1}).stabilityIndex - preSI];

                                % Logit transformation
                                phase = [];
                                phase = resultsEEG.(condition{1,1}).phaseR;
                                phase = log(phase ./ (1-phase));
                                phaseCoupling = [phaseCoupling; phase - preCoupling];
                            end
...
% % Sync only
resultsTable = table(ID, Game, Movement, Load, syncAccuracy, syncConsistency, power, stabilityIndex, phaseCoupling, Flexibility, Inhibition, workingMemory, ...
'VariableNames', {'ID', 'Game', 'Movement', 'Load', 'syncAccuracy', 'syncConsistency', 'Power', 'stabilityIndex', 'phaseCoupling', 'Flexibility', 'Inhibition', 'workingMemory'});
writetable(resultsTable, [pathResults '/All/statsTableSync_noOutliers.csv'])

% % BAASTA only
% resultsTable = table(ID, Game, musicConsistency, Flexibility, Inhibition, workingMemory, ...
% 'VariableNames', {'ID', 'Game', 'musicConsistency', 'Flexibility', 'Inhibition', 'workingMemory'});
